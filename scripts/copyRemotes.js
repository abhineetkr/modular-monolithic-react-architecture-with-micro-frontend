...
async function copyRemoteAssets() {
  try {
    console.log('📦 Copying remote assets...');
    ...
    if (await fs.pathExists(authDistPath)) {
      ...
      console.log('✅ Auth assets copied');
    } else {
      console.warn('⚠️ Auth dist folder not found. Make sure to build auth first.');
    }
    ...
    if (await fs.pathExists(dashboardDistPath)) {
      ...
      console.warn('⚠️ Dashboard dist folder not found. Make sure to build dashboard first.');
    }
    
    console.log('🎉 Remote assets copy completed!');
  } catch (error) {
    console.error('❌ Error copying remote assets:', error);
    process.exit(1);
  }
}
